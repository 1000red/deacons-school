...
class _TermsScreenState extends State<TermsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final crumb = '${widget.level.name} • ${widget.year.name}';
    return Scaffold(
      appBar: appBarFor(widget.year.name),
      body: Column(
        children: [
          ...
          Expanded(
            child: ListView.separated(
              ...
              itemBuilder: (context, i) {
                final term = terms[i];
                final path =
                    NavPath(level: widget.level, year: widget.year, term: term);
                return CustomList(
                  title: term.name,
                  subtitle: 'يحتوي على 5 مواد دراسية',
                  icon: Icons.menu_book,
                  color: widget.level.color.withValues(alpha: 0.9 - i * 0.12),
